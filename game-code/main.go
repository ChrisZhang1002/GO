...
//snake start on left-down with length of 8
func newSnake() *Snake {
...
// snake move action with in field #
func (snake *Snake) mov(x, y int) {
	// go to end of R,  start from L #
	if snake.x+x >= setwidth {
		snake.x = 0
	// go to end of L,  start from R #
	} else if snake.x+x < 0 {
		snake.x = setwidth - 1
	} else {
		snake.x += x
	}
	// go to end of U,  start from D #
	if snake.y+y >= setheight {
		snake.y = 0
	// go to end of D,  start from U #
	} else if snake.y+y < 0 {
		snake.y = setheight - 1
	} else {
		snake.y += y
